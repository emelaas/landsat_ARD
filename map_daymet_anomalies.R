...
mod <- c('Chill','Photo','SW Jan1','SW Mar17')
...
for (year in 1984:2017){
  ...
  plot(diff,col=my.colors3(150),zlim=c(-21,21),axes=FALSE,bty='n',legend.width=1.2,add=TRUE)
...
for (year in 1984:2017){
  ...
  colnames(diff_vals) <- c('Chill','Photo','SW Jan 1','SW Mar 17')
  
  Zscore <- raster(paste('/projectnb/modislc/projects/landsat_sentinel/ARD/tifs/agdd_Zscore_',year,'.tif',sep=""))
  Zscore_vals <- getValues(Zscore)
  
  diff_vals1 <- diff_vals[which(Zscore_vals>2 & below5_vals<=2),]
  diff_vals2 <- diff_vals[which(Zscore_vals>2 & below5_vals>2 & below5_vals<=5),]
  diff_vals3 <- diff_vals[which(Zscore_vals>2 & below5_vals>5),]
...
#   x11(h=4,w=11)
  ...
  if (length(diff_vals1)>4){
    ...
    plot(NA, xlim=c(-21,21), ylim=range(sapply(dens, "[", "y")),
      main=expression('0 ' <= 'No. Cold Winter Months < 3'),
  ...
  if (length(diff_vals2)>4){
    ...
    plot(NA, xlim=c(-21,21), ylim=range(sapply(dens, "[", "y")),
      main=expression('3 ' <= 'No. Cold Winter Months < 6'),
      xlab='Model Bias (days)')
    mapply(lines, dens, col=1:length(dens),lwd=4)
    abline(v=0,lty=2)
  }
  if (length(diff_vals3)>4){
    ...
    plot(NA, xlim=c(-21,21), ylim=range(sapply(dens, "[", "y")),
      main=expression('6 < No. Cold Winter Months' <= 8),
      xlab='Model Bias (days)')
    mapply(lines, dens, col=1:length(dens),lwd=4)
    abline(v=0,lty=2)
