...
in_dirs <- list.files(path=getwd(),
  pattern=glob2rx('RMSE*tif'),full.names=T,include.dirs=T,recursive=T)

#Get list of individual bands
rmse <- stack(in_dirs)

rmse_hex_all <- foreach(i = 1:length(r_reproj), .combine = rbind) %dopar% {
  print(i)    
  
  rmse_hex_mean <- c(r_reproj$proj_ID25k[i],matrix(NA,1,length(in_dirs)))
  
  r_extr <- extract(rmse,r_reproj[i,])
  if (sapply(r_extr,is.null)==1){
    ...
    rmse_hex_mean <- c(r_reproj$proj_ID25k[i],t(as.matrix(colMeans(r_extr[[1]],na.rm=TRUE))))
  }
  
  rmse_hex <- rmse_hex_mean
}

# Save model calibration results
save(rmse_hex_all,file='/projectnb/modislc/projects/landsat_sentinel/ARD/R_files/FIA_hex_rmse_mean')

load(file='/projectnb/modislc/projects/landsat_sentinel/ARD/R_files/FIA_hex_rmse_mean')
rmse_hex_all <- rmse_hex_all[,-c(4)]
...
rmse.group <- matrix(NA,5,4)
rmse.error.group <- matrix(NA,5,4)
for (i in 1:4){
  rmse.group[,i] <- aggregate(rmse_hex_all[,(i+1)],by=list(grp_max),FUN=mean,na.rm=TRUE)[,2]
  rmse.error.group[,i] <- aggregate(rmse_hex_all[,(i+1)],by=list(grp_max),FUN=sd,na.rm=TRUE)[,2]
}
...
#x11(h=5,w=8)
pdf(h=5,w=8,'/projectnb/modislc/projects/landsat_sentinel/ARD/figures/NCC/rmse_by_ftype.pdf')
barCenters <- barplot(t(rmse.group),beside=TRUE,
  names=c('OH','OG','EAC','MBB','AB'),ylim=c(0,14),
  col=colors, ylab='RMSE (days)',xlab='Major Forest Type')
