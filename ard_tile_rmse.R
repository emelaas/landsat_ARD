...
tot_pix_all <- foreach(i = 1:length(ard_shp), .combine = rbind) %dopar% {
  ...
  if (length(which(tile_extr[[1]]>0)>0)){
    tile_extr <- extract(stack(RMSE1,RMSE2,RMSE3,RMSE4,nobs),tile)
...
save(tot_pix_all,file='/projectnb/modislc/projects/landsat_sentinel/ARD/R_files/tot_pix_all')
...
cuts_s1 <- classIntervals(tot_pix2[,1],dataPrecision=3,style='fixed',
  fixedBreaks=c(3,5,7,9,11,15))
plotclr <- rev(brewer.pal(5,'PuOr'))
...
#x11(h=8,w=8)
pdf(h=8,w=8,'/projectnb/modislc/projects/landsat_sentinel/ARD/figures/NCC/ard_tile_rmse.pdf')
...
legend(1926712,-600000,title='',cex=0.785,legend=names(attr(colcode_s1,'table')),
