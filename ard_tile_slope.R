...
slope1 <- raster('slope1.tif')
slope2 <- raster('slope2.tif')
slope3 <- raster('slope3.tif')
slope4 <- raster('slope4.tif')
...
tot_pix_all <- foreach(i = 1:length(ard_shp), .combine = rbind) %dopar% {
  ...
  if (length(which(tile_extr[[1]]>0)>0)){
    tile_extr <- extract(stack(slope1,slope2,slope3,slope4,nobs),tile)
...
cuts_s1 <- classIntervals(tot_pix2[,1],dataPrecision=3,style='fixed',
  fixedBreaks=c(0,0.2,0.4,0.6,0.8,1.2,1.4,1.6,1.8,2))
...
#x11(h=8,w=8)
pdf(h=8,w=8,'/projectnb/modislc/projects/landsat_sentinel/ARD/figures/NCC/ard_tile_slope.pdf')
