...
for (yr in 1982:2017){
  ...
  for (j in 1:nrow(obs.SPR)){
    if (is.na(obs.SPR[j,2])==0 & obs.SPR[j,2]>60){
      MST[j,(yr-1981)] <- mean(tmean[j,(obs.SPR[j,2]-60):obs.SPR[j,2]])
    }
  }
}
...
o_mean <- apply(all.MST,1,mean,na.rm=TRUE)
o_sd <- apply(all.MST,1,sd,na.rm=TRUE)
o_mean_all <- replicate(36, o_mean)
o_sd_all <- replicate(36, o_sd)
o_Zscore <- (all.MST-o_mean_all)/o_sd_all

setwd(paste('/projectnb/modislc/projects/landsat_sentinel/ARD/',tile_name,'/PHENO_1KM/',sep=""))
save(all.MST,o_Zscore,file = "daymet_MST")
